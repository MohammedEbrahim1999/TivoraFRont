...
export default function SearchPage() {
    ...
    useEffect(() => {
        fetch(" http://localhost:1337/uploads/No_Data_86ad4502de.json")
            .then((res) => res.json())
    ...
    // ✅ load favorites directly from localStorage (no reset on refresh)
    ...
    // ✅ keep localStorage updated when favorites change
    useEffect(() => {
    ...
    const results = headerData.searchitem.filter(item =>
    ...
    return (
        <>
            <div className='py-4 px-3 shadow-sm' style={{ backgroundColor: theme.palette.background.breadcumbBG, color: theme.palette.text.primary }}>
                <Container>
                    <h2>{headerData?.searchFor} "{query}"</h2>
